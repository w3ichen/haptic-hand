...
void initMotorsGpio( void )
{
    ...
    GPIOPinAFConfig((GPIOTypeDef *)GPIOD_BASE_MORT, GPIOPinSource12, GPIO_AF_TIM4); 
    GPIOPinAFConfig((GPIOTypeDef *)GPIOD_BASE_MORT, GPIOPinSource13, GPIO_AF_TIM4); 
    GPIOPinAFConfig((GPIOTypeDef *)GPIOD_BASE_MORT, GPIOPinSource14, GPIO_AF_TIM4); 
...
void initHaplinkMotors( void )
{
    ...
    /* PWM1 Mode configuration: TIM3 Channel1 */
        // PWM Mode is either 1 or 2 (mode is unrelated to motor number). See Section 17.3.9
    TIM_OCInitStructure.TIM_OCMode = TIM_OCMode_PWM1_MORT;              // same for rest
    TIM_OCInitStructure.TIM_OutputState = TIM_OutputState_Enable_MORT;  // same for rest (removed redundant line)
    TIM_OCInitStructure.TIM_OCPolarity = TIM_OCPolarity_High_MORT;      // same for rest
    TIM_OCInitStructure.TIM_Pulse = TIM3_CCR1_Val;                      // CCR changes for each
...
/*******************************************************************************
  ...
  * @brief  Converts the desired torque to duty cycle based on calibration and
  *         outputs it on Motor7.
  * @param  double torque: the torque output desired on Motor 7 in Nm.
  * @retval None.
  */
