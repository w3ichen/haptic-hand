  ...
  if (!SIMULATION_MODE) {
    ...
    if (portList.length > SERIAL_PORT_INDEX) {
      ...
      try {
        ...
        // Force simulation mode if serial fails
        // Note: This requires sketch restart to take effect if SIMULATION_MODE was initially false
        // A cleaner way might involve setting a global 'serialAvailable' flag
...
void updateSerialData() {
  ...
  // Data reading is handled by serialEvent now
}
  ...
  if (messageString != null) {
    ...
    try {
      ...
      if (tokens.length == 13) {
        ...
        println("Serial Warning: Received incorrect number of tokens (" + tokens.length + "), expected 13.");
      }
