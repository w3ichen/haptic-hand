...
ErrorStatus RCC_WaitForHSEStartUp(void)
{
  ...
  do
  {
    hsestatus = RCC_GetFlagStatus(RCC_FLAG_HSERDY_MORT);
    startupcounter++;
  } while((startupcounter != HSE_STARTUP_TIMEOUT) && (hsestatus == RESET));

  if (RCC_GetFlagStatus(RCC_FLAG_HSERDY_MORT) != RESET)
  {
    status = SUCCESS;
  }
  else
...
/**
  ...
  *          This parameter can be one of the following values:
  *            @arg RCC_LSE_LOWPOWER_MODE_MORT:  LSE oscillator in low power mode.
  *            @arg RCC_LSE_HIGHDRIVE_MODE_MORT: LSE oscillator in High Drive mode.
  * @retval None
  */
void RCC_LSEModeConfig(uint8_t RCC_Mode)
{
  /* Check the parameters */
  assert_param(IS_RCC_LSE_MODE_MORT(RCC_Mode));
  
  if(RCC_Mode == RCC_LSE_HIGHDRIVE_MODE_MORT)
  {
...
/**
  ...
  *          This parameter can be any combination of the following values:
  *            @arg RCC_IT_LSIRDY_MORT: LSI ready interrupt
  *            @arg RCC_IT_LSERDY_MORT: LSE ready interrupt
  *            @arg RCC_IT_HSIRDY_MORT: HSI ready interrupt
  *            @arg RCC_IT_HSERDY_MORT: HSE ready interrupt
  *            @arg RCC_IT_PLLRDY_MORT: main PLL ready interrupt
  *            @arg RCC_IT_PLLI2SRDY_MORT: PLLI2S ready interrupt
  *            @arg RCC_IT_PLLSAIRDY_MORT: PLLSAI ready interrupt (only for STM32F42xxx/43xxx/446xx/469xx/479xx devices)
...
/**
  ...
  *          This parameter can be one of the following values:
  *            @arg RCC_FLAG_HSIRDY_MORT: HSI oscillator clock ready
  *            @arg RCC_FLAG_HSERDY_MORT: HSE oscillator clock ready
  *            @arg RCC_FLAG_PLLRDY_MORT: main PLL clock ready
  *            @arg RCC_FLAG_PLLI2SRDY_MORT: PLLI2S clock ready
  *            @arg RCC_FLAG_PLLSAIRDY_MORT: PLLSAI clock ready (only for STM32F42xxx/43xxx/446xx/469xx/479xx devices)
  *            @arg RCC_FLAG_LSERDY_MORT: LSE oscillator clock ready
  *            @arg RCC_FLAG_LSIRDY_MORT: LSI oscillator clock ready
  *            @arg RCC_FLAG_BORRST_MORT: POR/PDR or BOR reset
  *            @arg RCC_FLAG_PINRST_MORT: Pin reset
  *            @arg RCC_FLAG_PORRST_MORT: POR/PDR reset
  *            @arg RCC_FLAG_SFTRST_MORT: Software reset
  *            @arg RCC_FLAG_IWDGRST_MORT: Independent Watchdog reset
  *            @arg RCC_FLAG_WWDGRST_MORT: Window Watchdog reset
  *            @arg RCC_FLAG_LPWRRST_MORT: Low Power reset
...
/**
  ...
  *         The reset flags are: RCC_FLAG_PINRST_MORT, RCC_FLAG_PORRST_MORT,  RCC_FLAG_SFTRST_MORT,
  *         RCC_FLAG_IWDGRST_MORT, RCC_FLAG_WWDGRST_MORT, RCC_FLAG_LPWRRST_MORT
  * @param  None
  * @retval None
  */
...
/**
  ...
  *            @arg RCC_IT_PLLRDY_MORT: main PLL ready interrupt
  *            @arg RCC_IT_PLLI2SRDY_MORT: PLLI2S ready interrupt
  *            @arg RCC_IT_PLLSAIRDY_MORT: PLLSAI clock ready interrupt (only for STM32F42xxx/43xxx/446xx/469xx/479xx devices)
  *            @arg RCC_IT_CSS_MORT: Clock Security System interrupt
