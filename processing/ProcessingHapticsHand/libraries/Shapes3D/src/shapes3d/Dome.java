...
/**
 * This class represents a simple Dome (top half of an ellipsoid. It is the base class
 * for the SkyDome class.
 * 
 * @author Peter Lager
 *
 */
public class Dome extends Shape3D {
	 ...
	 */
	public Dome(float rad, int nbrSegs, int nbrSlices) {
	 ...
	 * @param nbrSlices number of slices in NS direction
	 */	
	public Dome(float radX, float radY, float radZ, int nbrSegs, int nbrSlices) {
	 ...
	 */
	public Dome(float radX, float radY, float radZ, int nbrSegs, int nbrSlices, Orientation orientation) {
		super();
		this.radX = radX;
		this.radY = radY;
		this.radZ = radZ;
	...
	protected void calcShape() {
		...
		double dAngleNS = 0.5 * Math.PI / nbrSlices;
		...
		for(int col = 0 ; col <= nbrSegs; col++) {
			...
			n[col][nbrSlices] = c[col][nbrSlices].normalize();
		}
